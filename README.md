...
2月8日
（一）迭代器
1、常用运算符i.begin():指向第一个元素，i.end():指向末元素之后。愚以为是为了使容器为空时二者指向同一位置，所以判断容器是否为空时可以直接 if(v.begin() != v.end())
2、C++中程序员在for循环中常用！=而不是>或<。原因是C++中所有容器都定义了==和！=，而只有string和vector有下标运算符（Qt的库中容器应该就强大多了）。
3、可以得到一个指向中间位置的迭代器：
auto mid = v.begin() + v.size()/2 ;
而对于顺序排列的容器，则可以以mid为工具使用二分法快速定位

2月17日
（一）运算符
1、*pName++等价于*(p++)即指向下一个位置（简洁是一种美德）
2、位操作符切忌和逻辑运算符搞混
位与（&）位或（|）位取反（~）位异或（^）;
逻辑与（&&）逻辑或（||）逻辑非（！）
